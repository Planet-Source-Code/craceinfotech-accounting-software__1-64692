...
Begin VB.Form AddTransaction1 
   ...
   Begin VB.TextBox txtParticular 
      ...
      TabIndex        =   11
      Text            =   "frmAddTransaction1.frx":0000
      Top             =   3900
      Width           =   4935
   End
   ...
   End
End
Attribute VB_Name = "AddTransaction1"
...
End Sub
Private Sub cboHead1_Validate(Cancel As Boolean)
If cbohead1.ListIndex = -1 Then
    'MsgBox Me.ActiveControl
    MsgBox "First you should select a Head"
    'Cancel = True
End If
...
Private Sub Form_Load()
  ...
  With adoMasterRS
    ...
    Do While Not .EOF
        ...
        cbohead1.AddItem !ACTITLE
        cbohead1.ItemData(cbohead1.NewIndex) = !ACNUMBER
...
Private Sub cmdCancel_Click()
    ...
    ViewTransaction1.RefreshTransaction
    ViewTransaction1.FillGridTrans
    Me.Hide
...
Private Sub ClearFields()
    ...
    cbohead1.ListIndex = -1
    txtDebit1 = "0.00"
    txtDebit1 = Format(Format(Val(txtDebit1), "0.00"), "@@@@@@@@@@")
    txtCredit1 = "0.00"
    txtCredit1 = Format(Format(Val(txtCredit1), "0.00"), "@@@@@@@@@@")
End Sub
...
With AddTransactionRS
    !entry_id = Val(txtEntryNo) + 1
    '!acn_date = Format(txtAcnDate, "dd/mm/yyyy")
    !acn_date = mskAcnDate.Text
    
    !ACNUMBER = cbohead1.ItemData(cbohead1.ListIndex)
    
    !debit = Val(txtDebit1)
    !credit = Val(txtCredit1)

    !particular = Trim(txtParticular)
    !cj = "C"
    CashInHand = CashInHand + Val(txtCredit1) - Val(txtDebit1)
...
ElseIf (Val(txtDebit) = 0 And Val(txtCredit) = 0) Then
    ...
    txtDebit.SetFocus
    Exit Sub
ElseIf cbohead1.ListIndex < 0 Then
    'IncompleteEntry = True
    MsgBox "Select a Head"
    cbohead1.SetFocus
    Exit Sub
ElseIf (Val(txtDebit1) = 0 And Val(txtCredit1) = 0) Then
    MsgBox "Both Amounts should not be empty"
    txtDebit1.SetFocus
    Exit Sub
...
End Sub
Private Sub txtCredit1_GotFocus()
txtCredit1 = Format(Val(txtCredit1), "0.00")
txtCredit1.SelStart = 0
txtCredit1.SelLength = Len(txtCredit1)

End Sub

Private Sub txtCredit1_KeyPress(KeyAscii As Integer)
    If (KeyAscii = vbKeyBack) Then Exit Sub
    If InStr("1234567890.", Chr(KeyAscii)) = 0 Then KeyAscii = 0
End Sub

Private Sub txtCredit1_LostFocus()
txtCredit1 = Format(Format(Val(txtCredit1), "0.00"), "@@@@@@@@@@")
End Sub

Private Sub txtCredit1_Validate(Cancel As Boolean)
    If Val(txtCredit1) > 0 Then
        If Val(txtDebit1) > 0 Then
            txtDebit1 = "0.00"
            txtDebit1 = Format(Format(Val(txtDebit1), "0.00"), "@@@@@@@@@@")
        End If
    Else
...
End Sub
Private Sub txtDebit1_GotFocus()
    txtDebit1 = Format(Val(txtDebit1), "0.00")
    txtDebit1.SelStart = 0
    txtDebit1.SelLength = Len(txtDebit1)
End Sub

Private Sub txtDebit1_KeyPress(KeyAscii As Integer)
    If (KeyAscii = vbKeyBack) Then Exit Sub
    If InStr("1234567890.", Chr(KeyAscii)) = 0 Then KeyAscii = 0
End Sub

Private Sub txtDebit1_LostFocus()
txtDebit1 = Format(Format(Val(txtDebit1), "0.00"), "@@@@@@@@@@")
End Sub

Private Sub txtDebit1_Validate(Cancel As Boolean)
    If Val(txtDebit1) > 0 Then
        If Val(txtCredit1) > 0 Then
            txtCredit1 = "0.00"
            txtCredit1 = Format(Format(Val(txtCredit1), "0.00"), "@@@@@@@@@@")
